...
async def test_deputies_by_name():
    ...
    print(f"PASS Search for non-existent name: {len(result_nonexistent.get('dados', [])) if result_nonexistent else 0} results")
...
async def test_deputy_details():
    ...
    if result and 'dados' in result and len(result['dados']) > 0:
        ...
        if details:
            ...
            print("FAIL Failed to retrieve deputy details")
    else:
